...
def autobounds_solver(edges, unobservables, csv_path):
    ...
    prog_ate = silent_run(lambda: problem.write_program(), output_file="outputs/autobounds_output.txt", new=True)
    prog_ate_optim = silent_run(lambda: prog_ate.run_scip(), output_file="outputs/autobounds_output.txt")

    # Extracting bounds
    lower_bound = np.round(prog_ate_optim[0]['dual'], 3)
    upper_bound = np.round(prog_ate_optim[1]['dual'], 3)
    write_output("==============================================")
    write_output(f"Causal effect lies in the interval [{lower_bound}, {upper_bound}]")
    write_output("==============================================")
    pass
